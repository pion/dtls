...
type Conn struct {
	...
	requireExtendedMasterSecret bool           // If we are a client should we require Extended Master Support extension
...
func createConn(nextConn net.Conn, flightHandler flightHandler, handshakeMessageHandler handshakeMessageHandler, config *Config, isClient bool) (*Conn, error) {
	...
	c := &Conn{
		...
		requireExtendedMasterSecret: config.RequireExtendedMasterSecret,
